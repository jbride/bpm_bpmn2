...
==== SINGLETON: start *Signal Subprocess Waitstate* 
...
==== SINGLETON: signal pInstances
This section of the reference architecture involves signalling the process instances that are now in a wait-state.
The SINGLETON KIE session is knowledgeable of all process instances for a deployment unit
