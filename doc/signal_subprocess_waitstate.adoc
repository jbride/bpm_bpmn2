...
In a terminal window in your local environment, execute the following command to instantiate an instance of the _signalsubprocess_ process definition :
...
Answer:  yes, the behavior of the process engine after having been bounced should be the same.
...
This strategy allows for concurrent requests to different BPMN2 instances to be processed simulataneously.
Subsequently, this strategy is ideal for many BPM use cases involving long-running processes.
